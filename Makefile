...
CLI_BASH_COMPLETIONS=npc.bash
CLI_ZSH_COMPLETIONS=_npc
CLI_FISH_COMPLETIONS=npc.fish
CLI_BASH_COMPLETIONS_FILE=$(RUST_RELEASE_BIN_DIR)/$(CLI_BASH_COMPLETIONS)
CLI_ZSH_COMPLETIONS_FILE=$(RUST_RELEASE_BIN_DIR)/$(CLI_ZSH_COMPLETIONS)
CLI_FISH_COMPLETIONS_FILE=$(RUST_RELEASE_BIN_DIR)/$(CLI_FISH_COMPLETIONS)
...
install: $(VARLINK_SRV_EXEC_RELEASE) $(CLI_EXEC_RELEASE)
	...
	sed -i -e 's/@INCLUDE_DIR@/$(subst /,\/,$(INCLUDE_DIR))/' \
		$(DESTDIR)$(PKG_CONFIG_LIBDIR)/$(CLIB_PKG_CONFIG_FILE)
	install -p -v -D -m755 $(CLI_BASH_COMPLETIONS_FILE) \
		$(DESTDIR)$(PREFIX)/share/bash-completion/completions/$(CLI_BASH_COMPLETIONS)
	install -p -v -D -m755 $(CLI_ZSH_COMPLETIONS_FILE) \
		$(DESTDIR)$(PREFIX)/share/zsh/site-functions/$(CLI_ZSH_COMPLETIONS)
	install -p -v -D -m755 $(CLI_FISH_COMPLETIONS_FILE) \
		$(DESTDIR)$(PREFIX)/share/fish/completions/$(CLI_FISH_COMPLETIONS)
...
uninstall:
	...
	- rm -fv $(DESTDIR)$(PREFIX)/share/bash-completion/completions/$(CLI_BASH_COMPLETIONS)
	- rm -fv $(DESTDIR)$(PREFIX)/share/zsh/site-functions/$(CLI_ZSH_COMPLETIONS)
	- rm -fv $(DESTDIR)$(PREFIX)/share/fish/completions/$(CLI_FISH_COMPLETIONS)
